...
> 2. ==并且极限是否能直接带值，也要分情况：加减不能带值，乘除可以。==

[极限是否可以带值详细情况：](https://gitee.com/Acido/images/raw/master/image/202211261347989.jpg)

<img src="https://gitee.com/Acido/images/raw/master/image/202211261347989.jpg" alt="极限可以直接带值情况" style="zoom: 80%;" />
...
> $$
> 性质4：函数$f(x)和g(x)$在区间$[a,b]$上总满足条件$f(x)\le g(x)$，则$\int_a^bf(x)dx\le\int_a^bg(x)dx$，当区间下限大于上限时，相反。
>
> 性质5：如果被积函数$f(x)=1$则：$\int_a^bdx=b-a$，等于积分长度
>
> 性质6(估值定理)：函数$f(x)$在区间$[a,b]$上的最大值与最小值分别为$M$与$m$，则$m(b-a)\le\int_a^bf(x)dx\le M(b-a)$
>
> ​			  根据性质$4、5、1$可得：
> $$
...
> $$
> 我们把==$f(\xi)=\frac{1}{b-a}\int_a^bf(x)dx$称为函数$f(x)$在区间$[a,b]$上的平均值==。此定理多用于计算小题的平均值。
>
> 性质8(积零偶倍)：$f(x)$为连续奇函数且区间$(a,b)$为**对称空间**：则$\int_a^bf(x)dx=0$，如：$\int_{-1}^1\frac{x}{1+x^2}=0$
>
> 性质9(积零偶倍)：$f(x)$在**对称空间**$(a,b)$上为偶函数，则：$\int_a^bf(x)=2\int_0^bf(x)dx$，如：$\int_{-1}^1f(x)dx=2\int_0^1f(x)dx$
>
> 性质10(定积分保号性)：如果函数$f(x)\ge0$在积分区间$[a,b](b>a)$恒成立，则定积分$\int_a^b f(x)dx>0$恒成立，反之也成立。
>
> 性质11：$\Big|\int_a^b f(x)dx\Big|\le\int_a^b\Big|f(x)\Big|dx$
...
- $Y$型积分
  ...
  \begin{equation*}
  	\begin{aligned}
  \\\Large{解：}\\
  &\quad 围成的图像如下，其图像可以看作x=y+4，x=\frac{1}{2}y^2，以及直线y=-2,y=4围成的Y型积分\\
...
> 就是在算出图形围成面积的基础上多$\pi$多平方。(整体乘$\pi$，被积函数**分别平方**)
>
> 如：
>
...
> 如果一个积分的横截面已知，那么对横截面$A(x)$的积分就是体积
...
> 定义：
>
> 1. 凡含有未知函数的导数(或微分)的方程，称为微分方程，如果未知函数是一元函数，则该方程为常微分方程$x,y,y'或y^{(n)}$；如果未知函数是多元函数，方程中出现未知函数的偏导数，则称为偏微分方程(不考)。即含有自变量，未知函数，及其导数或微分的方程。
...
微分方程中积分得出$ln\Delta$不用加绝对值符号。

### 2.2 一阶线性微分方程

> 判断线性微分方程条件：未知函数$y$不超过一次，且$y$的各阶导数也不超过一次。如：$xy'+2y''+x^2y=0(\surd)、(y')^2+5yy''+xy=0(\times)$
>
> 一阶线性微分方程通解解法：
>
