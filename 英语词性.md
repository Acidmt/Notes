...
- $n_1+$介词(with/together with/along with/as well as(和)/but(除了)/except/like)$+n_2+v$
  ...
  > 此时动词$v$是复数还是单数由于$n_1$决定。因为介词后面名词不参与句子成分。
  >
...
- More than one(多余，不止一个)、many a(不止一个)$+n$要用单数，$+v$也要用单数。
  ...
  >
  > 2. $It$ has been reported that more than one kid **has been infected** with the “Bird Flu”.
  >    据报道，不止一个孩子感染了“禽流感”。
  >
...
- $if$动词将来时态变化
  ...
  > $If$ the earth **exploded/should explode/were to explode** (explode) tomorrow，We would take (take) a holiday.
  > 如果明天地球爆照，我们就放假。(词根explo(用力往外)，explore.探索，explode.爆炸，exploit.开采)
...
>
> 与wish相同用法的关键词还有：as if(好像)、as though(好像)、if only(要是)
...
- 主语$+v_1+$as if/as though$+主+v_2$
  ...
  > 这里的动词($v_2$)变形和wish变形一样。即$v_2$要用虚拟。 
  >
