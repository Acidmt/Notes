...
定理三(函数极限的局部有界性)：若$\underset{x\to x_0}{lim}f(x)$存在则在$x_0$附近有界

定理四(函数极限的局部保号性)：若$\underset{x\to x_0}{lim}f(x)=A>0(或<0)$则在$x_0$附近函数$f(x)>0(或<0)$
...
>
> （4）如果$\lim\frac{b}{c}=1$，则称b与a是等价无穷小量，记作$a\sim b$
...
> $$
> 也就是说，反函数的导数等于其直接函数的导数的倒数(原函数导数和反函数的导数互为倒数)。
