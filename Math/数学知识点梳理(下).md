...
<img src="https://gitee.com/Acido/images/raw/master/image/202208291425546.png" alt="直角左边化为极坐标" style="zoom: 33%;" />
...
<img src="https://gitee.com/Acido/images/raw/master/image/202208291503175.jpg" alt="单抛物面围成图形-min" style="zoom:25%;" />
...
<img src="https://gitee.com/Acido/images/raw/master/image/202208292026458.png" alt="格林公式例1" style="zoom: 50%;" />
...
两类曲线积分关系如下：
$$
\Large \int_LP(x,y)dx+Q(x,y)dy=\int_L\Big[P(x,y)cos\alpha+Q(x,y)cos\beta\Big]ds
$$
其中$cos\alpha$和$cos\beta$是平面有向曲线$L$在点$(x,y)$处切线的方向余弦，即两类曲线积分关系式。

而$cos\alpha$、$cos\beta$和$cos\gamma$是空间曲线$L$在点$(x,y,z)$处切线的方向余弦。

$\Large例：把对坐标的曲线积分  \int_{L} P(x, y) d x+Q(x, y) d y  化成对弧长的曲线积分，\\\Large其中  L  为沿上半圆 周  x^{2}+y^{2}=2 x  从点  (0,0)  到点  (1,1) .$
$$
\begin{equation*}
	\begin{aligned}
\\\Large{解：}\\
&因为圆  x^{2}+y^{2}=2 x  ，即  (x-1)^{2}+y^{2}=1  ，故参数方程为\\
\\
&\left\{\begin{array}{l}
x=\cos \theta+1 \\
y=\sin \theta
\end{array}\right.\\
\\
&此处起点  (0,0)  对应的  \alpha=\pi  ，终点  \beta=\frac{\pi}{2} .\\
&因为  \alpha>\beta,故:\\
\\
&\cos \alpha=\frac{-x'}{\sqrt{x'^2+y^2}}=\frac{\sin \theta}{1}=\sin \theta=y, \\
\\
&\cos \beta=\frac{-y'}{\sqrt{x'^2+y'^2}}=\frac{-\cos \theta}{1}=-\cos \theta=1-x .\\
\\
&故由两类曲线积分的关系,有\\
\\
&\int_{L} P(x, y) d x+Q(x, y) d y=\int_{L}\Big[yP(x,y)+(1-x)Q(x,y)\Big] d s .

	\end{aligned}
\end{equation*}
$$
其中关于$cos\alpha$和$cos\beta$详细求法如下：
$$
&\Large注: 设有向曲线弧  L  的起点为  A  ，终点为  B . 曲线弧  L  由参数方程\\
...
&给出，起点  A  、终点  B  分别对应参数  \alpha, \beta . 则平面曲线  L  上的两类曲线积分之间有如下联系:\\

&\int_{L} P d x+Q d y=\int_{L}(P \cos \alpha+Q \cos \beta) d s\\

&其中  \alpha(x, y), \beta(x, y)  为有向曲线弧  L  在点  (x, y)  处的切向量的方向角. 其中\\
&当  \alpha<\beta  时，  \cos \alpha=\frac{\varphi^{\prime}(t)}{\sqrt{\varphi^{\prime 2}(t)+\psi^{\prime 2}(t)}}, \cos \beta=\frac{\psi^{\prime}(t)}{\sqrt{\varphi^{\prime 2}(t)+\psi^{\prime 2}(t)}} .\\
&当  \alpha>\beta  时，  \cos \alpha=\frac{-\varphi^{\prime}(t)}{\sqrt{\varphi^{\prime 2}(t)+\psi^{\prime 2}(t)}}, \cos \beta=\frac{-\psi^{\prime}(t)}{\sqrt{\varphi^{\prime 2}(t)+\psi^{\prime 2}(t)}} .
$$
...
> 若级数$\sum\limits_{n=1}^{\infty}u_n$收敛，则$\underset{n\to\infty}{lim}u_n=0$，也就是收敛极限的通项必定为$0$
>
...
补充：$\sum\limits_{n=2}^{\infty}\frac{1}{n(lnn)^p},此时n$次幂要为$1$，$p$为$lnn$的次幂。

$\sum\limits_{n=2}^{\infty}\frac{1}{n^p(lnn)}$，$lnn$次幂是$1$，$p$为$n$的次幂
...
$$
\Large\lim_{n\to\infty}\frac{u_n}{v_n}=\left\{ 
\begin{matrix}
...
\begin{aligned}
	\begin{gather*}
\\\Large{解：}\\
1.&找等价级数作为v_n,由n\to\infty,可知sin\frac{1}{n}\sim\frac{1}{n}\\
\\
...
- 当$x=x_1$时，级数$\sum\limits_{n=1}^\infty a_n(x-x_0)^n$发散，则：
  ...
  中心点为$x_0$及其中间区域不确定，$x_1$点和其对称点$\Delta$外区域都为发散域。
...
- 当$x=x_2$时，级数$\sum\limits_{n=1}^\infty a_n(x-x_0)^n$收敛，则：
  ...
  中心点为$x_0$，$x_2$点和其对称点$\Delta$所围成的区域为收敛域，且收敛域中的点都为绝对收敛。

- 当$x=x_3$时，$\sum\limits_{n=1}^\infty a_n(x-x_0)^n$条件收敛

  此时$x_3$和其对称点$\Delta$两边发散，中间包括$x_0$绝对收敛。

==注意：阿贝尔定理在对称两点处的敛散性不确定。==

$\Large 例:若\sum\limits{a_nx^n}在x=2处收敛,则该函数在x=-1处敛散性$
...
==几个重要麦克劳林公式：==
$$
\Large e^x=\sum\limits_{n=0}^{\infty}\frac{x^n}{n!}\quad x\in(-\infty,+\infty)\\
广义化:e^\omega=\sum\limits_{n=0}^{\infty}\frac{\omega^n}{n!},\omega\in(-\infty,+\infty)\\
\\
...
$$
规律：前三个带阶乘，$sinx$和$cosx$带奇偶之分，后两个没阶乘的$ln$带符号，另一个不带。
$$
\begin{array}{l}
\Large 麦克劳林及推导公式如下:\\
\\
