...
引力公式：$F=G·\frac{M·m}{r^2}$。$M$与$m$是两个物体的质量，$r$是两个质量$M,m$之间的距离。

常见的有一根细杆、圆盘对一个质点的引力，可用微元法将细杆(圆盘)划分为无数个点。

$\Large 例:$在$x\in[1,l]$上有一个线密度为$\ln x$的细杆，求该细杆对坐标原点处质量$m$点的引力

> 可用将细杆进行微元划分，设细杆上一点为$x$，则其中一段微元为$x,x+dx$。而密度$\ln x$乘微元长度$x+dx-x=dx$长度，即$\ln xdx$为细杆微元质量。

$$
\begin{equation*}
	\begin{aligned}
\\\Large{解:}\\
&由F=G·\frac{M·m}{r^2}可知\\
\\
&dF=\frac{m·\ln xdx}{x^2},之后在[1,l]累加即可得到引力\\
\\
&\int dFdx=\int \frac{m·\ln xdx}{x^2}=Gm·(1-\frac{\ln l}{l})
	\end{aligned}
\end{equation*}
$$

如果题中没有线密度，可用细杆质量$÷$细杆长度$=\rho$(线密度)
...
> 压强$P=\rho gh$.
>
> 其中$\rho$是液体密度，$g$是重力加速度，$h$是该点在液面下的深度。
>
> 静水压力$F=P·A=\rho gh·A$
...
- $f(x)=P_m(x)e^{\lambda x}$解法
  ...
  $\Large 例1:$求微分方程$y''-6y'+9y=(x+1)e^{3x}$的通解
  $$
  \begin{equation*}
  	\begin{aligned}
  \\\Large{解:}\\
  ...
  \end{equation*}
  $$
  $\Large 例2:$设二阶常系数线性微分方程$y''+\alpha y'+\beta y=\gamma e^x$的一个特解为$y^*=e^{2x}+(1+x)e^x$，确定常数$\alpha,\beta,\gamma$，并求该方程的通解。
  $$
  \begin{equation*}
  	\begin{aligned}
  \\\Large{解:}\\
  &由于y=Y_{齐次方程通解}+y^*_{非齐次方程特解}\\
  \\
  &\therefore 线性微分方程特解是齐次方程Y中C_1和C_2确定+y^*\\
  \\
  &即y^*=[C_1e^{r_1x}+C_2e^{r_2x}]+y_0①\\
  \\
  &或y^*=C_1e^{rx}+C_2xe^{rx}②,C_1,C_2是已知常数\\
  \\
  &故题中y^*=e^{2x}+(1+x)e^x=e^{2x}+e^x+xe^x\\
  \\
  &由于自由项为\gamma e^x,故y_0=e^x·a·x^k,故只能是y^*中后两项之一\\
  \\
  &且通解Y的两个项是线性无关的,形式只能是①②中的一个\\
  \\
  &能满足这个条件只能是e^{2x}和e^x,\therefore y_0=xe^x\\
  \\
  &从而可知特征方程根r_1=2,r_2=1\Longrightarrow (r-2)(r-1)=0\\
  \\
  &故r^2-3r+2=0,即\alpha=-3,\beta=2\\
  \\
  &由于y_0=xe^x是方程特解,将其代入题中所给方程,得\gamma=-1\\
  \\
  &\therefore 方程通解y=C_1e^{2x}+C_2e^x+xe^x
  	\end{aligned}
  \end{equation*}
  $$
...
- $f(x)=e^{\lambda x}(Acos\omega x+Bsin\omega x)$解法
  ...
  $\Large 例:$求微分方程$y''-4y'+8y=\sin x$的特解
  $$
  \begin{equation*}
  	\begin{aligned}
  \\\Large{解:}\\
...
（1）如果解中含特解$e^{rx}$，则$r$至少为单实根

（2）如果解中含特解$x^{k-1}e^{rx}$，则$r$至少为$k$重实根

（3）如果解中含特解$e^{ax}\cos\beta x$或$e^{ax}\sin\beta x$，则$a\pm\beta i$至少为单复根

（4）如果解中含特解$e^{ax}x\cos\beta x$或$e^{ax}x\sin\beta x$，则$a\pm\beta i$至少为二重复根

$\Large 例:$已知某四阶常系数齐次线性微分方程有特解$y_1(x)=e^x\cos2x,y_2(x)=x$，且方程中$y^{(4)}$前得系数为$1$，求该方程
$$
\begin{equation*}
	\begin{aligned}
\\\Large{解:}\\
&由e^x\cos2x\Longrightarrow r_{1,2}=1\pm 2i\\
\\
&由x=x·e^{0·x}\Longrightarrow r_{3,4}=0\\
\\
&故y^{(4)}=[r-(1+2i)][r-(1-2i)](r-0)^2=0\\
\\
&即y^{(4)}-2y'''+5y''=0
	\end{aligned}
\end{equation*}
...
形如$x^2\frac{d^2y}{dx^2}+px\frac{dy}{dx}+qy=f(x)$得方程称为欧拉方程，其中$p$与$q$为常数，$f(x)$为已知的连续函数，欧拉方程有固定解法。
...
1. 当$x>0$时，令$x=e^t$，则$t=\ln x,\frac{dt}{dx}=\frac{1}{x}$，于是
   ...
   \\
   &\frac{d^2y}{dx^2}=\frac{d}{dx}(\frac{1}{x}\frac{dy}{dt})=-\frac{1}{x^2}\frac{dy}{dt}+\frac{1}{x}\frac{d}{dx}(\frac{dy}{dt})\\
   \\
...
\begin{equation*}
	\begin{aligned}
\\\Large{解:}\\
&令x=e^t,原式=\frac{d^2y}{dt^2}+3\frac{dy}{dt}+2y=0\\
\\
&r^2+3r+2=0\Longrightarrow r_1=-1,r_2=-2\\
\\
&y=C_1e^{-t}+C_2e^{-2t}=C_1\frac{1}{x}+C_2\frac{1}{x^2}
	\end{aligned}
\end{equation*}
$$
...
$\Large 例:$设自行车前轮和后轮与地面的接触点分别为$P$和$Q$，并设$|PQ|=1$，起始时刻$P$在原点，$Q$在$(1,0)$点，若前轮沿$y$轴的正方向前进，求$Q$点的运动轨迹。
$$
\begin{equation*}
	\begin{aligned}
\\\Large{解:}\\
&如下图所示,当P点沿着y轴向上移动,记Q点轨迹形成\\
\\
&曲线y=y(x),并设曲线上Q点的坐标(x,y),P点坐标\\
\\
&为(0,Y),由|PQ|=1,得\\
\\
&x^2+(y-Y)^2=1,即y-Y=-\sqrt{1-x^2},(y>Y取负)\\
\\
&由题意可知,QP的方向就是曲线y=y(x)在(x,y)点的切线方向\\
\\
&故\frac{dy}{dx}=\frac{y-Y}{x}=-\frac{\sqrt{1-x^2}}{x}\\
\\
&两边积分:y=-\int\frac{\sqrt{1-x^2}}{x}dx=\ln\frac{1+\sqrt{1-x^2}}{x}-\sqrt{1-x^2}+C\\
\\
&由x=1时,y=0,得C=0.故Q点的轨迹方程为:\\
\\
&y=\ln\frac{1+\sqrt{1-x^2}}{x}-\sqrt{1-x^2}
	\end{aligned}
\end{equation*}
$$
<img src="https://image.sybblogs.fun/img-common/202407201303715.png" alt="微分方程例题" style="zoom:33%;" />

> 这类问题可以归纳为追踪问题，即一点$B$始终盯着目标点$A$运动。所以上图中，点$Q$目标点是$P$，故切线方程(上图直角斜边)即为$QP$方向。

### 6.2 微分方程物理应用

$\Large 例1:$飞机在机场降落时，为了减少滑行距离，在触地瞬间，飞机尾部张开减速伞，以增大阻力，使飞机迅速减速并停下，现有一质量为$m$的飞机，着陆时的水平速度为$v(0)$。经测试，减速伞打开后，飞机所受的总阻力与飞机的速度成正比。比例系数为$k,k>0$。从飞机接触跑道开始计时，设$t$时刻飞机滑行距离为$x(t)$，速度为$v(t)$，求飞机滑行的位移方程。
$$
\begin{equation*}
	\begin{aligned}
\\\Large{解:}\\
...
&故m·\frac{dv}{dt}=-kv\\
\\
&其中\frac{dv}{dt}=\frac{dv}{dx}·\frac{dx}{dt}=\frac{dv}{dx}·v\\
\\
&\therefore m·\frac{dv}{dx}·v=-kv\\
\\
&即mdv=-kdx\xRightarrow{分离常数}dx=-\frac{m}{k}dv\\
\\
&\xRightarrow{两边积分得}x=-\frac{m}{k}·v+C\\
\\
&由于v_0=v(0),x(0)=0\xRightarrow{代入得}-\frac{m}{k}·v(0)+C=0\\
\\
&故x=-\frac{m}{k}·v+\frac{m}{k}·v(0)
	\end{aligned}
\end{equation*}
$$

> 此题中，加速度$a=\frac{d^2x}{dt^2}=\frac{dv}{dt}=\frac{dv}{dx}·\frac{dx}{dt}=v·\frac{dv}{dx}$，这是因为选项均为$x$与$v$的关系式，故用相关变化率的手段写成上述表达式形式。

# 十三. 无穷级数

无穷级数的本质是数列的极限，所以等价无穷小等极限的性质可以直接使用。但要注意级数要为无穷小。如$\sum\limits_{n=2}^{\infty}\sin n$就没有等价无穷小，因为$n\to\infty$，同时还要注意的是等价的部分必须为因子(与其他部分是乘除关系，幂指型就不能等价).
