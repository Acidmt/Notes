...
\\
&则\sqrt{x_n(3-x_n)}\le\frac{x_n+3-x_n}{2}=\frac{3}{2}=x_{n+1}\\
\\
...
定理一：若数列$\{a_n\}$收敛，则其任何子列$\{a_{nk}\}$都收敛，且$\underset{k\to\infty}{\lim}a_{nk}=\underset{n\to\infty}{\lim}a_n$.

推论：$\underset{n\to\infty}{\lim}a_n=a\Longleftrightarrow\underset{k\to\infty}{\lim}a_{2k}=a$，且$\underset{k\to\infty}{\lim}a_{2k-1}=a$.

此定理提供了一个判断数列发散的方法：对于一个数列$\{a_n\}$，如果能找到一个发散的子列，则原数列一定发散；如果能找到至少两个收敛的子列$\{a_{nk}\}$和$\{a_{n'k}\}$，但它们收敛到不同极限，则原数列也一定发散。
