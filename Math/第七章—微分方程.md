...
[^3-1-1]:数乘后再相加
[^3-1-2]:两个量相除结果为常数$k$，则线性相关。如:$y和kx$。反之，两个量相除结果不为常数$k$，则线性无关。如：$x和x^2$
...
形如：$f(x)=e^{\lambda x}(Acos\omega x+Bsin\omega x)$方程称为二阶非齐次方程第二类型。

构成：指数函数、三角函数、多项式

特解：$y^*=x^k\Big[M_l(x)cos\omega+N_l(x)sin\omega x\Big]e^{\lambda x}$

其中：$cos\omega、sin\omega和e^{\lambda x}$为照抄项。$M_l和N_l$为一般项，一般项为$M_l和N_l$的最高次幂的通式。即如果最高次幂为2次，两个一般项为$Ax^2+Bx+c和Dx^2+Ex+F$

但考试只考：$y^*=(Acos\omega x+Bsin\omega x)e^{\lambda x}$
...
1.&先求y''+y=0的通解Y:\\
\\
&特征方程:r^2+1=0,解得r_1=r_2=\pm i=0\pm i\\
\\
&Y=e^{0x}(c_1cosx+c_2sinx)=c_1sosx+c_2sinx\\
\\
2.&原式可写为:y''+y=x(cos2x+0sin2x)e^{-x}=[xcos2x+0sin2x]e^{-x}\\
\\
&则y^*=x^k\Big[(Ax+B)cos2x+(Cx+D)sin2x\Big]e^{-x}\\
\\
&其中,\lambda+\omega i=-1+2i,不是特征方程的特征根\Longrightarrow k=0\\
\\
&y^*=\Big[(Ax+B)cos2x+(Cx+D)sin2x\Big]e^{-x}\\
\\
&代入原方程求得A、B、C、D的值。得处特解。
	\end{gather*}
\end{aligned}
$$
...
方程2(非齐次方程)：$y''+p(x)y'+q(x)y=f_1(x)\ne0$

方程3(非齐次方程)：$y''+p(x)y'+q(x)y=f_2(x)\ne0$

> 定理一：齐次方程两个解($y_1和y_2$)，两个解线性组合[^3-1-1]，即$(c_1y_1+c_2y_2)$，此时仍是线性方程得解。
> 				特别的：齐次方程两个线性无关[^3-1-2]的解$(y_1和y_2)$进行线性组合$(c_1y_1+c_2y_2)$，则是齐次方程的通解。
>
> 定理二(叠加定理)：方程2和3特解为$y_1^*和y^*_2$，则方程4：$y''+p(x)y'+q(x)y=f_1(x)+f_2(x)$的特解为$y^*_1+y^*_2$
